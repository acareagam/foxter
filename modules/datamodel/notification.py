...
class EmailTemplate(BaseModel):
    ...
    def set_fixtures(self):
        ...
        <p>
           Esta es una notificacion  {{=event_type}} de <a href='www.foxter.co'>Foxter</a>
        </p>
        Foxter news
        </html>
        """
        ...
        <p>
           Bienvenido a Foxter
           <br/>
           <a href='www.foxter.co'>ve atu perfil y completa tus datos!</a>
        </p>
        <p>
        ...
        <p>
           Click en el link para verificar tu correo
           <br/>
           <a href= '{{=CURL('person', 'account', args=['verify_email'], scheme=True, host=True)}}/%(key)s'>click para verificar tu correo </a>
        </p>
        <p>
            esta es una notificacion de Foxter
        </p>
        ...
        <html>
          ...
          <p>
           ...
           <br/>
           <a href='{{=CURL('person', 'account', args=['reset_password'], scheme=True, host=True)}}/%(key)s'>
        ...
        for key, subject in self.default_keys.items():
            if not self.db(self.entity.template_key == key).count():
                self.entity.insert(
                    template_key=key,
                    plain_text="Foxter Notification",
        ...
        if not self.db(self.entity.template_key == 'welcome_on_register').count():
                self.entity.insert(
                    ...
                    subject_text="Bienvenido a Foxter",
                )
        if not self.db(self.entity.template_key == 'verify_email').count():
                self.entity.insert(
                    ...
                    subject_text="Foxter - Verify your email",
                )
        if not self.db(self.entity.template_key == 'reset_password').count():
                self.entity.insert(
                    ...
                    subject_text="Foxter cambiar tu contraseña",
                )
